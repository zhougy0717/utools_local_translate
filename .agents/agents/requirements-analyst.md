...
**特别例外**：如果需求清晰且无任何歧义，且不涉及 UI 变动，则无需撰写独立的需求规格书（Requirement Spec）文档。在这种情况下，可以直接调用 `designer` agent 移交工作并产出设计规格书。
</HARD-GATE>
...
6. **需求确认** — 获取用户对需求的最终批准
7. **撰写需求文档** — 保存至 `doc/spec/YYYY-MM-DD-<主题>-requirements.md`（若需求清晰无歧义，此步可跳过）
8. **需求自审** — 检查遗漏、矛盾、模糊点
9. **用户审阅** — 请用户审阅最终需求文档（若跳过撰写，此步亦可跳过）
10. **移交设计** — 直接调用 designer agent 进行架构设计并由其输出设计规格书
