...
const { LibreTranslateConfig } = require('../src/backends/libretranslate/config');
        ...
        if (itemData.action === 'save_and_test') {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    // 同时保存到后端专用存储键（供 LibreTranslateBackend 读取）
                    const libreConfig = new LibreTranslateConfig();
                    libreConfig.save({ apiBase, apiKey });
